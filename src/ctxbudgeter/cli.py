...
def scan(
    ...
    output: Optional[Path] = typer.Option(None, "--output", "-o", help="Write JSON inventory."),
    emit_pack: Optional[Path] = typer.Option(
        None, "--emit-pack",
...
def compile_cmd(
    ...
    model: str = typer.Option("claude-sonnet-4.6", "--model", "-m"),
    system: Optional[Path] = typer.Option(None, "--system", "-s"),
    ignore: list[str] = typer.Option(DEFAULT_IGNORE, "--ignore"),
    max_files: int = typer.Option(200, "--max-files"),
    format: str = typer.Option("text", "--format", "-f", help="text | markdown | json."),
    output: Optional[Path] = typer.Option(None, "--output", "-o"),
    prompt_output: Optional[Path] = typer.Option(None, "--prompt-output"),
    save_pack: Optional[Path] = typer.Option(None, "--save-pack"),
...
def pack(
    ...
    output: Optional[Path] = typer.Option(None, "--output", "-o"),
    save_pack: Optional[Path] = typer.Option(None, "--save-pack"),
    fail_below: Optional[int] = typer.Option(
        None, "--fail-below",
...
def _emit_pack_yaml(
    ...
    lines.append(f"# Adjust priorities / cache_policy / required fields before use.")
    ...
    for f in inventory:
        ...
        lines.append(f"    compressible: true")
        lines.append("")
    out_path.write_text("\n".join(lines))
